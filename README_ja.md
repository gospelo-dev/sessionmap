...
| 列 | 意味 |
|---|---|
| MEM | エージェントプロセス自身が確保しているメモリ（[どの指標か](#メモリはどの指標か)） |
| TREE | 同じ指標を子プロセス（MCP サーバー、hook など）まで含めて合計したもの |
| UP | プロセス起動からの経過時間 |
...
1. プロセステーブル（sysinfo）からエージェントのプロセスを見つけます — メモリ / CPU / 起動時刻。PID の再利用は起動時刻で照合して弾きます
...
| プラットフォーム | 指標 | 理由 |
|---|---|---|
| macOS | **物理フットプリント**（`proc_pid_rusage`） | カーネルは触られないプロセスのページを圧縮するため、メモリを保持したままでも放置セッションの RSS は下がります。フットプリントは圧縮ページとスワップ分も数えます — アクティビティモニタの「メモリ」列と同じ値です |
| Linux | RSS | 既定の構成ではメモリコンプレッサーが存在しません |
| Windows | プライベートワーキングセット（sysinfo 経由） | ページアウトされたプライベートメモリを最初から含みます |

これは**このツールが本来見つけたい相手にこそ効きます**。メモリ逼迫下のマシンで、7 時間放置された OpenCode セッションは **RSS 218 MB に対しフットプリント 1.4 GB** でした。RSS でソートすると、実際に使用中のセッションより下に並んでしまいます。

`--json` のフィールド名は互換性のため `rss_self` / `rss_tree` のままで、中身は上表で選ばれた指標です。
